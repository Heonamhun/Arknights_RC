...
import tkinter.font as tkfo
import random

root = tk.Tk()

root.title("Arknights Random Challenge")
root.geometry("660x400+50+50")
root.resizable(True,True)
ico=tk.PhotoImage(file='C:\\Users\\PNT\\Desktop\\ARC\\ico\\555.ico')
root.iconphoto(False, ico)
    
def actions1():
    Message=''
    op = [0 for i in range(12)]
    ...
    b = ["4-1", "4-2", "4-3", "4-4", "4-5", "4-6", "4-7", "4-8", "4-9", "4-10", "S4-1", "S4-2", "S4-3", "S4-4", "S4-5", "S4-6", "S4-7", "S4-8",
...
##    print(len(a))
    ...
    if 'EX' in st:
        ...
        danger = 1
    elif hard == 1 and 64 <= c <= 86:
        danger = 1
                
    #오퍼 편성(EX, S, 스토리8지 이상일시 11명편성)
                #(그이하일시 12명 그대로편성)
    if danger == 0 and Act_Sni.get() == False:
        Message += "\n"
        for i in range(0, 12):
            c = random.randint(0,252)
            op.insert(i, a[c])
            ...
            for j in range(0,i):
                if op[i] == op[j]:
                    i-=1
                    break
    elif danger == 0 and Act_Sni.get() == True:
        Message += "\n"
        ...
        for i in range(1,12):
            ...
            for j in range(1,i):
                if op[i] == op[j]:
                    ...
                    i-=1
                    break
    elif danger == 1 and Act_Sni.get() == False: #고난이도 편성
        Message += "고난이도 감지, 6성 1개 필수편성\n"
        ...
        for i in range(1,12):
            ...
            for j in range(1,i):
                if op[i] == op[j]:
                    ...
                    i-=1
                    break
    elif danger == 1 and Act_Sni.get() == True:
        Message += "고난이도 감지, 6성 1개 필수편성\n"
        c = random.randint(0,59)
        op.insert(0, star6[c])
...
font=tkfo.Font(family="맑은고딕", size=10)
Act_Sni=tk.BooleanVar()
chb=tk.Checkbutton(root,
                   text="스나이퍼 필수편성(체크시 랜덤스나1명 무조건편성)",
                   font=font,
                   variable=Act_Sni)
chb.pack(side="top")

font=tkfo.Font(family="맑은고딕", size=13, weight="bold")
lt=tk.StringVar()
lt.set("아래 'Start' 버튼을 눌러 시작하세요")
l=tk.Label(root, textvariable=lt, font=font)
